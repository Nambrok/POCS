...
double hypothesis(ml::version1::matrix x, int ligne, ml::vector theta)
{
        return theta[0] * x(ligne, 0) + theta[1] * x(ligne, 1) + theta[2] * x(ligne, 2);
}

double computeCost(ml::version1::matrix X, ml::vector y, ml::vector theta)
{
        auto J = 0.0;
        for(auto i = 0; i < m; ++i) {
...
double computeMean(ml::version1::matrix X, int i)
{
        assert(X.size_y() < i);
        double mu = 0.0;
...
double computeStd(ml::version1::matrix X, double mu, int i)
{
        auto sigma = 0.0;
        for(auto j = 0u; j < X.size_x(); ++j) {
                std::cout << "Dans la boucle " << j << std::endl;
...
void featureNormalize(ml::version1::matrix X, double mu, double sigma, int i)
{
        for(auto j = 0; j < m; ++j) {
...
void gradientDescent(ml::version1::matrix X, ml::vector y, ml::vector theta, double alpha, int iteration)
{
        ...
        for(auto iter = 0; iter<iteration; ++iter) {
                ...
                //TODO: Faut faire update sur tout les y, pas forcément 3
                ml::vector Update(3);
                Update[0] = 0.0; Update[1] = 0.0; Update[2] = 0.0;
                for(auto i = 0; i<m; ++i) {
                        const auto h = hypothesis(X, i, theta) - y[i];
...
int main()
{
        ml::version1::matrix X(m, 3); ml::vector y(m);
        ...
        std::cout << "Lancement de computeMean" << std::endl;

        auto mu = computeMean(X,2);

        std::cout << "Lancement de computeStd" << std::endl;

        auto sigma = computeStd(X,mu,2);

        std::cout << "Affichage mu" << std::endl;
        std::cout << mu << std::endl;

        std::cout << "Affichage sigma" << std::endl;
        std::cout << sigma << std::endl;

...
double hypothesis(const ml::version1::matrix& X, int ligne, const ml::vector& theta)
{
...
void gradientDescent(ml::version1::matrix& X, ml::vector& y, ml::vector& theta, double alpha, int iteration)
{
        ...
        for(auto iter = 0; iter<iteration; ++iter) {
                ...
                for(auto i = 0u; i<X.size_x(); ++i) {
                        const auto h = hypothesis(X, i, theta) - y[i];
                        Update[0] = Update[0] + alpha/X.size_x() * h * X(i,0);
                        Update[1] = Update[1] + alpha/X.size_x() * h * X(i,1);
                        Update[2] = Update[2] + alpha/X.size_x() * h * X(i,2);
                }
...
int main()
{
        ...
        auto predict1 = hypothesis(x1, 0, theta);
        ...
        auto predict2 = hypothesis(x2, 0, theta);
        ...
        x3(0,1) = 1.0;
        x3(0,2) = (168 - mu) / sigma;
        auto predict3 = hypothesis(x3, 0, theta);
        ...
        x4(0,1) = 1.0;
        x4(0,2) = (195 - mu) / sigma;
        auto predict4 = hypothesis(x4, 0, theta);
